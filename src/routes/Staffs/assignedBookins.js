...
import {branchBookings,deleteBooking,bookingRest,serviceListData, ResetSaved, CompletedStaffBookings, AssignedStaffBookings,CancelledBookings,CompletedBookingStaffReport} from "../../appRedux/actions";
...
const Title = (props) => {
  ...
  const onDelete =(key,val) =>{
   ...
   // setLoading(false);
   setTimeout(() => {
    dispatch(AssignedStaffBookings());
  ...
  if(BookingList === ''){
      dispatch(AssignedStaffBookings(msearch));
 ...
 // setCount('called')
  ...
  const onFinish = values => {
   ...
   setLoading(true);
   dispatch(AssignedStaffBookings(values.mobilenumber));
    setTimeout(() => {
      setLoading(false);
  ...
  return (
    ...
    serviceAll.map((service, index) => (
        <Option value={service.Id}>{service.ServiceName}</Option>
          ...
          <div><br></br>
          <TextArea rows={4}  onChange={(val)=>setDescription(val)} style={{ width: '100%' }} placeholder="Please enter Description"  /></div>
        </Modal>
  ...
  //return;
    ...
    setTimeout(() => {
      dispatch(AssignedStaffBookings());
      dispatch(CompletedStaffBookings());
      dispatch(CompletedBookingStaffReport());
      
      props.history.push('/staff/completedBookings');
    }, 3000);
