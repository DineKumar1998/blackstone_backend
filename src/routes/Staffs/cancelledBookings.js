...
import {branchBookings,deleteBooking,bookingRest,serviceListData, ResetSaved, CancelledStaffBookings} from "../../appRedux/actions";
...
const Title = () => {
  ...
  const BookingList = useSelector(({auth}) => auth.cancelledBookings);
  ...
  if(BookingList === ''){
      dispatch(CancelledStaffBookings(msearch));
 ...
 // setCount('called')
  ...
  const onFinish = values => {
   ...
   setLoading(true);
   dispatch(CancelledStaffBookings(values.mobilenumber));
    setTimeout(() => {
      setLoading(false);
  ...
  function refreshall(){
    dispatch(CancelledStaffBookings());
    setLoading(true);
    setTimeout(() => {
      setLoading(false);
     //  dispatch(bookingRest());
  ...
  return (
    <div className="ant-card ant-card-bordered" >
      <Card className="gx-card" title={'CANCELLED BOOKINGS'}>
       <Form
      {...formItemLayout}
      form={form}
      name="register"
