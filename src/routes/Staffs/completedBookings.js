...
import {branchBookings,deleteBooking,bookingRest,serviceListData, ResetSaved, CompletedStaffBookings} from "../../appRedux/actions";
...
const Title = () => {
  ...
  },
  {
  ...
  }, 
   {
    ...
    dataIndex: 'Room_name',
    render:(text, record)=>(
      <p style={{color:"#fff"}}>{record.Room_name}</p>
  )
  }, 
 {
    title: 'Total Amount',
  ...
  if(BookingList === ''){
      dispatch(CompletedStaffBookings(msearch));
 ...
 // setCount('called')
  ...
  const onFinish = values => {
   ...
   setLoading(true);
   dispatch(CompletedStaffBookings(values.mobilenumber));
    setTimeout(() => {
      setLoading(false);
