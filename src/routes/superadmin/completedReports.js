...
import {branchBookings,deleteBooking,bookingRest,serviceListData, ResetSaved, CompletedBookings,CompletedBookingsDatewise,superadminDashboardData} from "../../appRedux/actions";
...
const Title = () => {
  ...
  const BookingList = useSelector(({auth}) => auth.completedReports);
 ...
 
  {
    ...
    summary: {
      render: (text, record) => {
        return <div>Summary: {record.reduce((sum, record) => sum + record.Amount, 0)}</div>
      },
 ...
 const pinnedData = [
  ...
  if(BookingList === ''){
      dispatch(CompletedBookingsDatewise(msearch));
 ...
 // setCount('called')
  ...
  const onFinish = values => {
   ...
   //return;
   setLoading(true);
   dispatch(CompletedBookingsDatewise(startDate,endDate));
    setTimeout(() => {
      setLoading(false);
  ...
  function refreshall(){
    dispatch(CompletedBookingsDatewise());
    setLoading(true);
    setTimeout(() => {
      setLoading(false);
     //  dispatch(bookingRest());
  ...
  return (
    <div className="ant-card ant-card-bordered" >
      <Card className="gx-card" title={"COMPLETED BOOKINGS REPORT"}>
       <Form
      {...formItemLayout}
      form={form}
      name="register"
    ...
    <Card title="">
      ...
      <Table className="gx-table-responsive"
            ...
            // footer={false}
             exportableProps={{ showColumnPicker: false,fileName: "Black_Stones_Completed_Reports",btnProps: {
              type: "primary",
              icon:false,
             ...
             summary={pageData => {
              ...
              return (
                  ...
                  <td class="ant-table-cell"><p style={{color:'#f9813a'}}>{(DashBoard!='')?DashBoard.TotalSales:0} SAR</p></td>
                  <td class="ant-table-cell"></td>
                  <td class="ant-table-cell"></td>
    </tr>
